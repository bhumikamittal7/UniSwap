...
const exchange_name = 'byExchange';           
const token_name = 'AshokuCoin';               
const token_symbol = 'YB';             
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    if (amountEth <= 0) {
      alert("Please enter a valid amount of ETH to add.");
      return;
    }
    let rates = await getLiquidity(maxSlippagePct);
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    if (amountEth < 0) {
      alert("Please enter a valid amount of ETH to remove.");
      return;
    }
    let rates = await getLiquidity(maxSlippagePct);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).removeLiquidity(ethers.utils.parseUnits(String(amountEth), "wei"), rates.max_exchange_rate, rates.min_exchange_rate);
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    if (amountToken <= 0) {
      alert("Please enter a valid amount of tokens to swap.");
      return;
    }
    let rates = await getLiquidity(maxSlippagePct);
    await token_contract.connect(provider.getSigner(defaultAccount)).increaseAllowance(exchange_address, amountToken);
    await exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(ethers.utils.parseUnits(String(amountToken), "wei"), rates.max_exchange_rate);
}
...
async function getLiquidity(maxSlippagePct) {
